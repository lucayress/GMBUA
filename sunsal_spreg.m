function [z,res_p,res_d] = sunsal_spreg(M,y,Xh,beta,varargin)

%% [x] = sunsal_v2(M,y,Edg,Xh,beta,varargin)
%
...
%
%         min  (1/2) ||M X-y||^2_F + lambda ||X||_1 + beta ||Edg .* (Xh-X)||^2_F
%          X              
%
...
% just least squares
%---------------------------------------------
if sum(sum(lambda == 0)) && beta == 0 &&  strcmp(positivity,'no') && strcmp(addone,'no')
    z = pinv(M)*y;
...
if  strcmp(addone,'yes') && strcmp(positivity,'no') && beta == 0 
    F = M'*M;
...
%---------------------------------------------
if  strcmp(addone,'yes') && strcmp(positivity,'no') && beta ~= 0 
    error('Spatial regularization not implemented for FCLS!')
end
...
while (i <= AL_iters) && ((abs (res_p) > tol1) || (abs (res_d) > tol2)) 
    ...
    if strcmp(addone,'yes')
       ...
       error('new regularization not implemented in this case')
    else
%        x = IF*(yy+mu*(z+d));
%        for j=1:N
%            x(:,j) = (MtM+mu*I+beta*diag((Edg(:,j).^2))) \ (yy(:,j)+mu*(z(:,j)+d(:,j)) + beta*(Edg(:,j).^2).*Xh(:,j));
%        end
        x = IF*(yy+mu*(z+d) +beta*Xh);
    end
    ...
    if mod(i,10) == 1
        ...
        if  mu_changed
           ...
           % dont do anything since it is faster
%            IF = UF*diag(1./(sF+mu))*UF';
           IF = UF*diag(1./(sF+mu+beta))*UF';
           Aux = IF*B'*inv(B*IF*B');
           x_aux = Aux*a;
